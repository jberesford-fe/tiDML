...
# Define the recipe for outcome
treatment_recipe <- recipes::recipe(~ .) |>     # Generic template
  recipes::step_log(disp, hp, offset = 1) |>
...
fit <- dml_plr(
  ...
  m_model = rf_classif,
  g_model = rf_reg,
  recipe_m = treatment_recipe,
  recipe_g = treatment_recipe
)
